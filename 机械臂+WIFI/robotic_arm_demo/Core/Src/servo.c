...
//void translate_angle_to_pulse(double angle_1,double angle_2,double angle_3,double angle_4,double angle_5,double angle_6)
//{
//	pulse_1 = ((((45-fabs(angle_1)) + 90) / 90 ) + 0.5)*(20000/20);//参与姿态解算，控制旋转角度，范围（-90，90）
//	pulse_2 = (((angle_2 + 45) / 90 ) + 0.5)*(20000/20);//参与姿态解算，控制倾转角度，范围（-90，90）
//	pulse_3 = (((angle_3 + 90) / 90 ) + 0.5)*(20000/20);//参与姿态解算，控制倾转角度，范围（-90，90）
//	pulse_4 = (((angle_4 + 45) / 90 ) + 0.5)*(20000/20);//参与姿态解算，控制倾转角度，范围（-90，90）
//	pulse_5 = (((angle_5 + 90) / 90 ) + 0.5)*(20000/20);//控制末端控制器旋转角度，可以忽略，范围（-90，90）
//	pulse_6 = (((angle_6 + 90) / 90 ) + 0.5)*(20000/20);//控制末端控制器抓取，范围（-90，90）
//}
...
j1 =  - atan(target_x / (target_y + bottom_r)) * (57.3);
...
void servo_catch(double servo_target_angle_1,double servo_target_angle_2,double servo_target_angle_3,double servo_target_angle_4)
{
	if(servo_target_angle_1>0)
	{
	 for(b1=0;b1<=servo_target_angle_1;b1++)
		{
		pwm_out(b1,0,0,0,0,0);
    HAL_Delay(50);
		}
	}
	else if(servo_target_angle_1<0)
	{
	 for(b1=0;b1>=servo_target_angle_1;b1--)
		{
		pwm_out(b1,0,0,0,0,0);
    HAL_Delay(50);
 	...
 	//2号舵机
	for(b2=0;b2<=servo_target_angle_2;b2++)
	 {
		  	pwm_out(b1,b2,0,0,0,0);
	    delay_ms(10);
	 } 
  //3号
	for(b3=0;b3<=servo_target_angle_3;b3++)
	 {
			pwm_out(b1,b2,b3,0,0,0);
      delay_ms(10);
	 }	 

	 //4号舵机
	 	for(b4=0;b4>=servo_target_angle_4;b4--)
	 {
		pwm_out(b1,b2,b3,b4,0,0);
		 delay_ms(10);
	 }
	 
	  	for(b6=0;b6<=90;b6++)
	 {
		 pwm_out(b1,b2,b3,b4,b6,b6);
		 delay_ms(10);
	 }
...
void servo_fangzhi_blue(double pulse_1,double pulse_2,double pulse_3,double pulse_4)
{
	//4号舵机
	for(b4=pulse_4;b4<=0;b4++)
	 {
		 pwm_out(b1,b2,b3,b4,b6,b6);
		 delay_ms(10);
	 }
...
void servo_fangzhi_yellow(double pulse_1,double pulse_2,double pulse_3,double pulse_4)
{
	//4号舵机
	for(b4=pulse_4;b4<=0;b4++)
	 {
...
{
    servo_control(-90,30,0,30,45,90);
    HAL_Delay(500);
}
...
{
    servo_control(-90,30,0,30,45,160);
    HAL_Delay(1000);
}

void servo_transfer_yellow(void)
{
    servo_control(-90,30,0,30,45,20);
    HAL_Delay(1000);
}
...
//	servo_reset();
//	HAL_Delay(5000);
	pwm_out(45, 45, 45, 45, 0, 45);
	HAL_Delay(5000);
//	servo_reset();
//	HAL_Delay(5000);

	pwm_out(90, 90, 90, 90, 0, 90);
	HAL_Delay(5000);
//	servo_reset();
//	HAL_Delay(5000);
	pwm_out(-45, -45, -45, -45, 0, -45);
...
void blue_task(void)
{        OLED_ShowCHinese(32,3,18);//可回收垃圾
			OLED_ShowCHinese(48,3,19);			
			OLED_ShowCHinese(64,3,16);
			OLED_ShowCHinese(80,3,17);
	      HAL_Delay(1000);
			servo_catch(0,50,60,0);
			//pwm_out(0,45,45,20,0,0);
			servo_fangzhi_blue(0,50,60,0);
			HAL_Delay(1000);
			servo_drop();
...
{
	OLED_ShowCHinese(32,3,13);//可回收垃圾
			OLED_ShowCHinese(48,3,14);
			OLED_ShowCHinese(64,3,15);
	      ...
	      HAL_Delay(1000);
			servo_catch(0,50,60,0);
			//pwm_out(0,45,45,20,0,0);
			servo_fangzhi_yellow(0,50,60,0);
			HAL_Delay(1000);
			servo_drop();
...
void servo_test1(void)
{
	  pwm_out(0,0,0,0,0,0);
   delay_ms(1000);
    pwm_out(0,0,0,45,0,0);
   delay_ms(1000);
	 pwm_out(0,0,0,90,0,0);
   delay_ms(1000);
	pwm_out(0,0,0,-45,0,0);
   delay_ms(1000);
	pwm_out(0,0,0,-90,0,0);
